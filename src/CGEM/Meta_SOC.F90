      Subroutine Meta_SOC(f,T,S,h,dT)

      use cgem, only: nf,SDay,ws,iOM1_A,iOM1_Z,iOM1_R,iOM1_BC, &
       & iO2,iNO3,iNH4,isx1A,isy1A,isx1Z,isy1Z,sy1R,sx1R,sx2R,sx1BC,sy1BC 
...
!Then convert from C to N
      F_POM_A = -SDay*ws(iOM1_A)*f(iOM1_A)*f(isy1A)/f(isx1A)
      F_POM_Z = -SDay*ws(iOM1_Z)*f(iOM1_Z)*f(isy1Z)/f(isx1Z)
      F_POM_R = -SDay*ws(iOM1_R)*f(iOM1_R)*sy1R/sx1R
      F_POM_BC = -SDay*ws(iOM1_BC)*f(iOM1_BC)*sy1BC/sx1BC
      F_POM = F_POM_A + F_POM_Z + F_POM_R + F_POM_BC  
!      write(6,'(*(g0,:,", "))') "FP,FPA,FPZ,FPR,FPBC,FPOM",F_POM,F_POM_A,F_POM_Z,F_POM_R,F_POM_BC 
...
!Subtract OM flux, with conversion back to C from N
      f(iOM1_A) = AMAX1(f(iOM1_A)  - (f(isx1A)/f(isy1A))*F_POM_A/h*dT/Sday,0.)
      f(iOM1_Z) = AMAX1(f(iOM1_Z)  - (f(isx1Z)/f(isy1Z))*F_POM_Z/h*dT/Sday,0.)
      f(iOM1_R) = AMAX1(f(iOM1_R)  - (sx1R/sy1R)*F_POM_R/h*dT/Sday,0.)
      f(iOM1_BC) = AMAX1(f(iOM1_BC)  - (sx1BC/sy1BC)*F_POM_BC/h*dT/Sday,0.)
