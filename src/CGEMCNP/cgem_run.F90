...
subroutine cgem_run(istep,myrank)
  ...
  do i=1,nea
    ...
    if(lat*lon.le.0.0) then
      lat = cgemlat
      lon = cgemlon
    endif
    if(debug.eq.1.and.istep.eq.1) write(6,*) "x,y,lat,lon",ylat_el(i),xlon_el(i),lat,lon
    ...
    !This bit is from ICM...accounting for volumes, since we are using concentrations
    if(do_inflation) then !Inflation coef (ratio of volumes)
      ...
      zrat=htot/dzz1
      if(debug.eq.1.and.i.eq.10) write(6,*) "zrat,htot,dzz1",abs(zrat-1),htot,dzz1,zrat
